...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.items = GetResourceState('mt-inventory') ~= 'missing' and exports['mt-inventory']:LoadInventory(PlayerData.source, PlayerData.citizenid) or {}
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.GetCardSlot(cardNumber, cardType)
        ...
        local slots = exports['mt-inventory']:GetSlotsByItem(self.PlayerData.items, item)
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        ...
        if GetResourceState('mt-inventory') ~= 'missing' then exports['mt-inventory']:SaveInventory(source) end
...
function QBCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        ...
        if GetResourceState('mt-inventory') ~= 'missing' then exports['mt-inventory']:SaveInventory(PlayerData, true) end
...
function QBCore.Player.SaveInventory(source)
    if GetResourceState('mt-inventory') == 'missing' then return end
    exports['mt-inventory']:SaveInventory(source, false)
end

function QBCore.Player.SaveOfflineInventory(PlayerData)
    if GetResourceState('mt-inventory') == 'missing' then return end
    exports['mt-inventory']:SaveInventory(PlayerData, true)
end

function QBCore.Player.GetTotalWeight(items)
    if GetResourceState('mt-inventory') == 'missing' then return end
    return exports['mt-inventory']:GetTotalWeight(items)
end

function QBCore.Player.GetSlotsByItem(items, itemName)
    if GetResourceState('mt-inventory') == 'missing' then return end
    return exports['mt-inventory']:GetSlotsByItem(items, itemName)
end

function QBCore.Player.GetFirstSlotByItem(items, itemName)
    if GetResourceState('mt-inventory') == 'missing' then return end
    return exports['mt-inventory']:GetFirstSlotByItem(items, itemName)
end
