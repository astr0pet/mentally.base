...
local function RegisterApartmentEntranceZone(apartmentID, apartmentData)
    local entranceCoords = apartmentData.coords['enter']
    ...
    for _, coords in ipairs(entranceCoords) do
...
local function RegisterApartmentEntranceTarget(apartmentID, apartmentData)
    ...
    for _, coords in ipairs(entranceCoords) do
        local boxName = 'apartmentEntrance_' .. apartmentID .. '_' .. tostring(coords.x) .. '_' .. tostring(coords.y) .. '_' .. tostring(coords.z)
        local boxData = apartmentData.polyzoneBoxData

        if not boxData.created then
            local options = {}
                ...
                options = {
                    {
                        ...
                        entranceID = apartmentID,  -- Pass entrance ID to distinguish entrances
                    },
                }
            else
                options = {
            ...
            print("Box Zone Added:", boxName)  -- Add a debug print statement
...
local function SetClosestApartment()
    ...
    for id, apartment in pairs(Apartments.Locations) do
        if apartment and apartment.coords and apartment.coords.enter then
            local entranceCoords = apartment.coords.enter
            for _, coords in ipairs(entranceCoords) do
                local distcheck = #(pos - vector3(coords.x, coords.y, coords.z))

                if not current or distcheck < dist then
                    current = id
                end
            end
        else
            print("Missing or invalid data for apartment", id)
        end
    end

    if current and current ~= ClosestHouse and LocalPlayer.state.isLoggedIn and not InApartment then
...
RegisterNetEvent('apartments:client:EnterApartment', function(data)
    ...
    QBCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result ~= nil then
            EnterApartment(entranceID, result.name)
        end
    end)
end)
